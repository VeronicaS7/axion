...
async function check() {
  ...
  try {
    ...
    console.log("Databases on 5433:", dbs);
    await client.end();
    ...
    for (const db of dbs) {
      ...
      try {
        ...
        if (tables.includes('questions')) {
           ...
           const resCols = await dbClient.query(`SELECT column_name FROM information_schema.columns WHERE table_name = 'questions' AND column_name = 'needs_image_review'`);
           out += `has_needs_image_review: ${resCols.rows.length > 0}\n`;
        }
        ...
        const snaps = tables.filter(t => t.startsWith('snapshot_') && t.endsWith('_backup'));
        out += `snapshots: ${snaps.join(', ')}\n`;
    ...
    console.error('Connection error on 5433', err.stack);
  }
}
